...
static void _wt_put_on_stream_error(void* ctx, void* error) {
  ...
  wt_connection_send_error(pipeline->connection, CLIENT_API_STATUS_INTERNAL_ERROR, message);
...
static void _wt_handle_put(wt_connection_t* conn, cbor_item_t* frame) {
  ...
  if (block_cache_can_fit(conn->bc, required) != CACHE_FIT_OK) {
    wt_connection_send_error(conn, CLIENT_API_STATUS_INTERNAL_ERROR,
  ...
  switch (type) {
    ...
    case CLIENT_API_EPHEMERAL_LIST_REQUEST: {
      ...
      /* MARK_PERMANENT's announce is skipped on this transport: rep announce
         ...
         wt_transport_set_peer_node (the node serves the friend and
...
wt_connection_t* wt_connection_create(wt_transport_t* transport, HQUIC connection, HQUIC stream) {
  ...
  /* Peer/friend/bootstrap handlers borrow the daemon's node from the
     ...
     wt_transport_set_peer_node, and peer ops then reply with an error. */
  conn->peer_ctx.conn = (block_connection_t*)conn;
  conn->peer_ctx.network = transport->peer_node ? transport->peer_node->network : NULL;
  conn->peer_ctx.authority = transport->peer_node ? transport->peer_node->authority : NULL;
  conn->peer_ctx.actor = &conn->actor;
  conn->peer_ctx.is_authenticated = conn->is_authenticated;
  conn->peer_ctx.send_frame = (block_send_frame_fn)wt_connection_send_frame;
  conn->peer_ctx.send_error = (block_send_error_fn)wt_connection_send_error;
