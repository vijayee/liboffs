...
static void _ws_put_on_stream_error(void* ctx, void* error) {
  ...
  _ws_connection_send_error(pipeline->connection, CLIENT_API_STATUS_INTERNAL_ERROR, message);
...
static void _ws_handle_put(ws_connection_t* conn, cbor_item_t* frame) {
  ...
  if (block_cache_can_fit(conn->bc, required) != CACHE_FIT_OK) {
    _ws_connection_send_error(conn, CLIENT_API_STATUS_INTERNAL_ERROR,
...
static void _ws_dispatch_frame(ws_connection_t* conn, uint8_t type, cbor_item_t* frame) {
  switch (type) {
    ...
    case CLIENT_API_HEALTH_REQUEST: {
      ...
      _ws_connection_send_frame(conn, health_frame);
      free(json_str);
      break;
    }
    default:
