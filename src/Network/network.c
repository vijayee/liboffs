...
static void network_handle_salutation(network_t* network, message_t* msg,
  ...
  if (peer != NULL) {
    ...
    if (pending->peer_addr.ss_family == AF_INET) {
      ...
      /* platform_address_t.inet.addr is network byte order (the setter swaps
         to the legacy host-order u32 itself); port is host byte order. */
...
static void network_handle_find_block(network_t* network, message_t* msg) {
  ...
  fprintf(stderr, "NET-TRACE: find_block_execute result=%d hops=%zu\n", result, next_hop_count);
