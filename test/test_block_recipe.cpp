...
TEST(NewBlocksRecipe, PullCreatesBlock) {
  ...
  char* path = (char*)"test_newblocksrecipe_pull";
  rm_rf(path);
  mkdir_p(path);
...
TEST(NewBlocksRecipe, PullMultipleBlocks) {
  ...
  char* path = (char*)"test_newblocksrecipe_multi";
  rm_rf(path);
  mkdir_p(path);
...
TEST(RecyclerRecipe, PullFromDescriptor) {
  ...
  char* path = (char*)"test_recyclerrecipe_pull";
  rm_rf(path);
  mkdir_p(path);
...
TEST(BlockRecipeIntegration, WriteableOffStreamWithNewBlocksRecipe) {
  ...
  char* wstream_path = (char*)"test_recipe_integration_wstream";
  rm_rf(wstream_path);
