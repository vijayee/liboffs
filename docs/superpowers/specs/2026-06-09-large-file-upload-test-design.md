...
- **WebTransport (WT)** needs TLS certs; the fixture generates them on the fly in-process via the project's libcrypto (the same library used by `tools/offs-ca/ca_ops.c`): `EVP_PKEY_keygen` for an RSA-2048 key, `X509_new` / `X509_set_pubkey` / `X509_sign` for the self-signed v3 cert, `PEM_write_X509` / `PEM_write_PrivateKey` to write the PEM files. No shelling out to the system `openssl`. The PEM output is byte-equivalent to what `openssl req -x509 -newkey rsa:2048 -days 1 -nodes -subj /CN=liboffs-test` would produce.
